...
void LoRaInit(void)
{
	...
	gpio_pullup_en(SX126x_SPI_SELECT);
...
#endif
	SetPaConfig(0x04, 0x07, 0x00, 0x01); // PA Optimal Settings +22 dBm
	SetOvercurrentProtection(120.0);  // current max 60mA for the whole device
	SetPowerConfig(txPowerInDbm, SX126X_PA_RAMP_200U); //0 fuer Empfaenger

	uint8_t clamp_cfg;
	ReadRegister(SX126X_REG_TX_CLAMP_CONFIG, &clamp_cfg, 1);
	printf("TX_CLAMP_CONFIG: 0x%02X\n", clamp_cfg);
	clamp_cfg |= 0x1E;
	WriteRegister(SX126X_REG_TX_CLAMP_CONFIG, &clamp_cfg, 1);
...
void LoRaConfig(uint8_t spreadingFactor, uint8_t bandwidth, uint8_t codingRate, uint16_t preambleLength, uint8_t payloadLen, bool crcOn, bool invertIrq) 
{
	...
	uint8_t ldro = 1; // LowDataRateOptimize OFF
...
void SetSleep(uint8_t mode) {
	uint8_t data = mode;
	WriteCommand_NoTimeout(SX126X_CMD_SET_SLEEP, &data, 1); // 0x84
}
...
void SetOvercurrentProtection(float currentLimit)
{
	if((currentLimit >= 0.0) && (currentLimit <= 140.0)) {
		uint8_t buf[1];
		buf[0] = 0x38; // (uint8_t)(currentLimit / 2.5);
...
// WriteCommand with retry
void WriteCommand_NoTimeout(uint8_t cmd, uint8_t* data, uint8_t numBytes) {
	uint8_t status;
	// for (int retry=1; retry<10; retry++) {
	// 	ESP_LOGD(TAG, "status=%02x", status);
	// 	if (status == 0) break;
	// 	ESP_LOGW(TAG, "WriteCommand2 status=%02x retry=%d", status, retry);
	// }
	status = WriteCommand2_NoTimeout(cmd, data, numBytes);
	if (status != 0) {
		ESP_LOGE(TAG, "SPI Transaction error:0x%02x", status);
		LoRaError(ERR_SPI_TRANSACTION);
	}
...
uint8_t WriteCommand2_NoTimeout(uint8_t cmd, uint8_t* data, uint8_t numBytes) {
	// ensure BUSY is low (state meachine ready)
	WaitForIdle(BUSY_WAIT);
	...
	// New By imliubo
	uint8_t wdata[2] = {0x84, 0x00};
	spi_write_byte(wdata, 2);
