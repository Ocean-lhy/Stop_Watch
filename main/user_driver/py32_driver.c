...
static io_expander_handle_t *py32_handle = NULL;
...
static void print_all_pin_states(const char *stage)
{
    ...
    ret = io_expander_gpio_get_mode_reg(py32_handle, &gpio_mode);
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "GPIO模式寄存器: 0x%04X", gpio_mode);
    }
    
    ret = io_expander_gpio_get_output_reg(py32_handle, &gpio_output);
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "GPIO输出寄存器: 0x%04X", gpio_output);
    }
    
    ret = io_expander_gpio_get_input_reg(py32_handle, &gpio_input);
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "GPIO输入寄存器: 0x%04X", gpio_input);
    }
    
    ret = io_expander_gpio_get_pull_up_reg(py32_handle, &gpio_pu);
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "GPIO上拉寄存器: 0x%04X", gpio_pu);
    }
    
    ret = io_expander_gpio_get_pull_down_reg(py32_handle, &gpio_pd);
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "GPIO下拉寄存器: 0x%04X", gpio_pd);
    }
    
    ret = io_expander_gpio_get_drive_reg(py32_handle, &gpio_drive);
    if (ret == ESP_OK) {
...
esp_err_t py32_init(i2c_bus_handle_t i2c_bus)
{
    ...
    // 配置I2C扩展器
    io_expander_config_t config = {.i2c_port       = I2C_NUM_0,  // 从i2c_bus获取端口号
                                    .device_address = 0x6F,
                                    .timeout_ms     = 1000};

    esp_err_t ret = io_expander_init(&config, &py32_handle);
    if (ret != ESP_OK) {
    ...
    // 输入引脚
    // io_expander_gpio_set_mode(py32_handle, PY32_VIN_DET_PIN, IO_EXP_GPIO_MODE_INPUT);
    ...
    // 初始化引脚状态
    // io_expander_gpio_set_level(py32_handle, PY32_SPK_EN_PIN, 0);         // 扬声器默认禁用
    // io_expander_gpio_set_level(py32_handle, PY32_MUX_CTR_PIN, 0);        // 默认连接U0串口
    // io_expander_gpio_set_level(py32_handle, PY32_AU_EN_PIN, 0);          // 音频默认禁用
    ...
    // io_expander_gpio_set_drive(py32_handle, PY32_MOTOR_EN_PIN, IO_EXP_GPIO_DRIVE_PUSH_PULL); // 电机PWM输出引脚需要设置为推挽输出

    // 禁用电机PWM
    // io_expander_pwm_set_duty(py32_handle, PY32_MOTOR_PWM_CHANNEL, 0, false, true);

    io_expander_set_i2c_config(py32_handle, 0, false, false, false);
    ...
    uint16_t voltage = 0;
    io_expander_get_ref_voltage(py32_handle, &voltage);
...
esp_err_t py32_deinit(void)
{
    ...
    esp_err_t ret = io_expander_deinit(py32_handle);
    py32_handle   = NULL;
...
esp_err_t py32_speaker_disable(void)
{
    ...
    ESP_LOGI(TAG, "禁用扬声器");
    return io_expander_gpio_set_level(py32_handle, PY32_SPK_EN_PIN, 0);
}
...
esp_err_t py32_lcd_reset(void)
{
    ...
    // 拉低复位引脚
    esp_err_t ret = io_expander_gpio_set_level(py32_handle, PY32_OLED_RST_PIN, 0);
...
esp_err_t py32_mux_set_mode(py32_mux_mode_t mode)
{
    ...
    return io_expander_gpio_set_level(py32_handle, PY32_MUX_CTR_PIN, level);
}
...
esp_err_t py32_mux_get_mode(py32_mux_mode_t *mode)
{
    ...
    uint8_t level;
    esp_err_t ret = io_expander_gpio_get_level(py32_handle, PY32_MUX_CTR_PIN, &level);
    if (ret == ESP_OK) {
...
esp_err_t py32_motor_set_pwm(uint8_t duty_percent)
{
    ...
    esp_err_t ret = io_expander_pwm_set_duty(py32_handle, PY32_MOTOR_PWM_CHANNEL, duty_percent, false, enable);
    if (ret != ESP_OK) {
    ...
    ret = io_expander_pwm_get_duty(py32_handle, PY32_MOTOR_PWM_CHANNEL, &read_duty, &read_polarity, &read_enable);
    if (ret != ESP_OK) {
...
esp_err_t py32_motor_enable(uint8_t duty_percent)
{
    ...
    // 设置PWM占空比并启用
    esp_err_t ret = io_expander_pwm_set_duty(py32_handle, PY32_MOTOR_PWM_CHANNEL, duty_percent, false, true);
    if (ret != ESP_OK) {
...
esp_err_t py32_motor_disable(void)
{
    ...
    // 设置PWM占空比为0并禁用
    esp_err_t ret = io_expander_pwm_set_duty(py32_handle, PY32_MOTOR_PWM_CHANNEL, 0, false, false);
    if (ret != ESP_OK) {
...
esp_err_t py32_motor_set_frequency(uint16_t frequency)
{
    ...
    esp_err_t ret = io_expander_pwm_set_frequency(py32_handle, frequency);
    if (ret != ESP_OK) {
    ...
    ret = io_expander_pwm_get_frequency(py32_handle, &read_frequency);
    if (ret != ESP_OK) {
...
esp_err_t py32_motor_get_frequency(uint16_t *frequency)
{
    ...
    esp_err_t ret = io_expander_pwm_get_frequency(py32_handle, frequency);
    if (ret == ESP_OK) {
...
esp_err_t py32_motor_get_pwm(uint8_t *duty_percent, bool *polarity, bool *enable)
{
    ...
    esp_err_t ret = io_expander_pwm_get_duty(py32_handle, PY32_MOTOR_PWM_CHANNEL, duty_percent, polarity, enable);
    if (ret == ESP_OK) {
...
esp_err_t py32_sleep(uint8_t sleep_mode)
{
    ...
    return io_expander_set_i2c_config(py32_handle, sleep_time, false, false, false);
}
...
esp_err_t py32_release_all(void)
{
    ...
    // 设置所有输出引脚为低电平
    ret |= io_expander_gpio_set_level(py32_handle, PY32_SPK_EN_PIN, 0);
    ret |= io_expander_gpio_set_level(py32_handle, PY32_MUX_CTR_PIN, 0);    // 默认连接U0串口
    ret |= io_expander_gpio_set_level(py32_handle, PY32_AU_EN_PIN, 0);  // 音频默认禁用
    ret |= io_expander_gpio_set_level(py32_handle, PY32_OLED_RST_PIN, 0);
    ret |= io_expander_gpio_set_level(py32_handle, PY32_L3B_EN_PIN, 0);
