...
esp_err_t py32_init(i2c_bus_handle_t i2c_bus)
{
    ...
    // for flash py32 firmware
    // ESP_LOGI(TAG, "进入Bootloader模式");
    // i2c_expander_bootloader_enter(py32_handle);
    // while (1) 
    // {
    ...
    // i2c_expander_set_i2c_config(py32_handle, 0, false, 0);
 ...
 * @brief 设置PY32睡眠模式
 */
esp_err_t py32_sleep(uint8_t wake_mode, uint8_t sleep_time)
{
    if (py32_handle == NULL) {
        return ESP_ERR_INVALID_STATE;
    }
    ...
    if (wake_mode == 0) {
        ESP_LOGI(TAG, "设置唤醒下降沿");
    }
    else {
        ESP_LOGI(TAG, "设置唤醒上升沿");
    }

    ESP_LOGI(TAG, "设置睡眠时间: %d", sleep_time);

    // 根据睡眠模式设置I2C扩展器配置
    // sleep_mode: 0=不休眠, 1-15=休眠时间
    uint8_t sleep_time_cfg = (sleep_time > 0) ? sleep_time : 0;

    return i2c_expander_set_i2c_config(py32_handle, sleep_time_cfg, true, wake_mode, true);
}
